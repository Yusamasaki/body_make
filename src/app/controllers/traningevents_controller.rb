class TraningeventsController < ApplicationController
  ...
  def create
    ...
    if @traningevent.save
      redirect_to user_traningevents_path(current_user, today_traning_day_id: @today_traning_day, start_date: params[:start_date], start_time: params[:start_time])
    else
      flash[:danger] = "登録に失敗しました。"
      redirect_to new_user_traningevent_path(current_user, today_traning_day_id: @today_traning_day, start_date: params[:start_date], start_time: params[:start_time])
    end
  end
