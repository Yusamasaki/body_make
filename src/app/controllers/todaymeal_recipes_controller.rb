class TodaymealRecipesController < ApplicationController
  ...
  def create
    ...
    now_recipe = @user.recipes.find(@todaymeal_recipe.recipe_id)
    ...
    if now_recipe.calorie.present?
      if todaymeal_recipe_valid.nil?
        ...
        flash[:danger] = "記録に失敗しました。#{params[:start_time]}の#{@timezone.time_zone}には#{@recipe.recipe_name}は登録してあります。分量などで調整下さい。"
      ...
      flash[:danger] = "記録に失敗しました。#{now_recipe.recipe_name}には食品が登録されていません。"
      redirect_to new_user_todaymeal_recipe_path(@user, switching: "record", recipe_id: params[:recipe_id], timezone_id: params[:timezone_id], start_date: params[:start_date], start_time: params[:start_time])
    end
  end
