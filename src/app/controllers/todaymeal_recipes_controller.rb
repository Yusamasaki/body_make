class TodaymealRecipesController < ApplicationController
  ...
  def create
    ...
    if @todaymeal_recipe.save!
      ...
      redirect_to user_recipes_path(@user, start_date: params[:start_date], start_time: params[:start_time])
    else
  ...
  def todaymeal_recipe_params
    params.require(:todaymeal_recipe).permit(:start_time, :recipe_id, :timezone_id)
  end
